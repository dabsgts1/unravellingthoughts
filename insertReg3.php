...
//REGISTERING USER
	...
	if(empty($password)||empty($conpassword) || empty($rname) || empty($name) || !($conpassword==$password) ||  !filter_var($email, FILTER_VALIDATE_EMAIL) || empty($email))
		{
			echo "<h2>ERROR</h2>";
	...
	else if($name && $rname && $password && $conpassword &&$conpassword==$password)
		{
			$sql ="SELECT * FROM users WHERE name='$name'";
			$result = $conn->query($sql);
			$count=$result->num_rows;
			...
			$n=1;
			$sql="INSERT INTO users(name, password, email, rname, teacher) VALUES('$name','$passwordmd5','$email' ,'$rname', '$n')";
